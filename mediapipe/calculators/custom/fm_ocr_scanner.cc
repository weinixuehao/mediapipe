...
#include <iostream>
//在具体调用的地方，看这几个常量的解释
const int kHoughLinesPThreshold = 50;
const double kHoughLinesPMinLinLength = 50.0;
const double kHoughLinesPMaxLineGap = 10.0;
...
//            std::cout << "average_corners i = " << i << ",  point is: " << corner.point << ", segment size is: " << corner.segments.size() << std::endl;
    ...
    if (average_corners.size() >= 4) {
        ...
        } else if (average_corners.size() == 4 && !strictMode) { // 备用 如果没有找到
            for (Corner corner : average_corners) {
                results.push_back(corner.point);
            }
        }
    }
